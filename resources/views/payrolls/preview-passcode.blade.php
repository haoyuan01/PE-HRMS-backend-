...
<style>
    ...
    .input-group input { width: 100%; padding: 12px; border-radius: 5px; border: 1px solid #d1d5db; font-size: 14px; }
    .passcode-inputs { display: flex; gap: 10px; justify-content: center; }
    .passcode-inputs input { width: 44px; height: 48px; padding: 0; text-align: center; font-size: 22px; font-weight: 600; }
    .button { display: block; width: 100%; padding: 14px; color: white; text-align: center; font-weight: 600; border: none; border-radius: 5px; cursor: pointer; font-size: 15px; background-color:#17a2b8; }
...
<div class="wrapper">
    <div class="container">
        ...
        <div class="content">
            ...
            <p>Please enter your 6 digit system PIN to preview your payroll.</p>
            <form method="POST" action="{{ $action_url }}">
                @csrf
                <input type="hidden" name="token" value="{{ $token }}">
                <input type="hidden" name="email" value="{{ $email }}">
                <input type="hidden" name="payroll_uuid" value="{{ $payroll_uuid }}">
...
<script>
    ...
    const updatePasscode = () => { passcode.value = Array.from(digits).map((input) => input.value).join(''); };
    digits.forEach((input, index) => {
        ...
        input.addEventListener('paste', (event) => {
            ...
            value.split('').forEach((number, pasteIndex) => { if (digits[pasteIndex]) { digits[pasteIndex].value = number; } });
            updatePasscode();
            if (digits[value.length - 1]) { digits[value.length - 1].focus(); }
        });
    });
</script>
</html>
